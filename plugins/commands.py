    ...
    if AUTH_CHANNEL:
        try:
            ...
            if btn:
                ...
                await message.reply_text(text=f"<b>👋 Hey {message.from_user.mention},\n\nJust Join the channel then click on try again button. & You Will Get Video/Files 😇</b>", reply_markup=InlineKeyboardMarkup(btn))
                return
        except Exception as e:
            print(e)
    username = (await client.get_me()).username
...
# Ask Doubt on telegram @KingVJ01
    ...
    if not files_:
        ...
        try:
            ...
            title = '@Future_Doctors_Region ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
